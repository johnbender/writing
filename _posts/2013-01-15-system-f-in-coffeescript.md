...
It adds a type requirement to the argument of abstractions. The CoffeeScript equivalent adds a function that compares a type argument to the argument term. To keep things simple, `ifft` will throw an exception when the types don't line up. This will be more interesting later with System F when the type `τ` can be defined after the fact instead of inside or with the abstraction.
